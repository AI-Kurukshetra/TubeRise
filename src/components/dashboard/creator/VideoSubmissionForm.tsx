...
export default function VideoSubmissionForm({ invitationId, campaignId }: VideoSubmissionFormProps) {
  ...
  const validate = () => {
    ...
    if (!videoId) return 'Invalid YouTube URL. Use a youtube.com link.'
    ...
    <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-100 p-6 space-y-5 max-w-2xl">
      <div>
        ...
        <input
          ...
          required
        />
        <p className="text-xs text-gray-400 mt-1">Accepts youtube.com links</p>
      </div>
      ...
      <div className="flex items-center gap-3">
        <button
          type="submit"
          disabled={isPending}
          className="inline-flex items-center gap-2 bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500 text-white px-4 py-2 rounded-lg text-sm font-medium hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md shadow-emerald-500/25"
        >
